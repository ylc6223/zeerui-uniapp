<template>
  <view v-if="_IsOpened" class="zui-toast">
    <view v-if="hasMask" class="zui-toast__overlay"></view>
    <view
        class="toast-body"
        :class="['zui-toast__body--custom-image' ? image : '', 'toast-body--text' ? !realImg && !icon : '', !!status ? `zui-toast__body--${status}` : '']"
        :style="customStyle"
        @tap="handleClick"
    >
      <view class="toast-body-content">
        ...
        <view v-if="isRenderIcon" class="toast-body-content__icon">
          <text class="zui-icon" :class="[`zui-icon-${icon}`]"/>
        </view>
...
export default {
  ...
  props: {
    ...
    // 元素的状态 可选值:error,loading,success
    status: {
      type: String,
      default: ''
    },
  ...
  computed: {
    realImg() {
      return this.image || this.statusImg[this.status] || null
    },
    isRenderIcon() {
      return !!(this.icon && !(this.image || this.statusImg[this.status]))
    },
    _IsOpened() {
      if (this.isOpened) {
        this.makeTimer(this.duration || 0)
      }
  ...
  methods: {
    ...
    handleClose() {
      this.$emit('close', this._IsOpened)
    },
    handleClick(e) {
...
.zui-toast {
  ...
  .toast-body {
    ...
    &.zui-toast__body--custom-image,
    &.zui-toast__body--success,
    &.zui-toast__body--error,
    &.zui-toast__body--loading {
