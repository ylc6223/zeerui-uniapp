<template>
	<view class="zui-button__wrap"
		:class="[(width==='100%' || !width || width===true) && (!btnSize || btnSize===true)?'zui-btn__flex-1':'',getShapeClass(shape, plain),!disabled?'zui-button__hover':'']"
		:style="{width: getWidth, height: getHeight, margin: margin}">
		<button class="zui-btn" :class="[
				plain ? 'zui-' + type + '-outline' : 'zui-btn-' + (type || 'primary'),
		...
		<view class="zui-button__border" :class="[getShapeClass(shape, plain),getDisabledClass(disabled, type, plain)]"
...
<script>
	export default {
		...
		computed: {
			...
			getHeight() {
				//medium 184*40 / small 120 40/ mini 58*32
				let height = this.height || (uni && uni.$zui && uni.$zui.zuiButton.height) || '96rpx';
				if (this.btnSize && this.btnSize !== true) {
					height = {
						'medium': '80rpx',
						'small': '80rpx',
			...
			getSize() {
				return this.size || (uni && uni.$zui && uni.$zui.zuiButton.size) || 32;
			}
		},
		data() {
			return {
		...
		methods: {
			...
			getDisabledClass: function(disabled, type, plain) {
				...
				if (disabled && type != 'white' && type.indexOf('-') == -1) {
					let classVal = this.disabledGray ? 'zui-gray-disabled' : 'zui-dark-disabled';
					className = plain ? 'zui-dark-disabled-outline' : classVal;
				}
				return className;
			},
			getShapeClass: function(shape, plain) {
				let className = '';
				if (shape == 'circle') {
					className = plain ? 'zui-outline-fillet' : 'zui-fillet';
				} else if (shape == 'rightAngle') {
					className = plain ? 'zui-outline-rightAngle' : 'zui-rightAngle';
				}
				return className;
			},
			getHoverClass: function(disabled, type, plain) {
				let className = '';
				if (!disabled) {
					className = plain ? 'zui-outline-hover' : 'zui-' + (type || 'primary') + '-hover';
				}
				return className;
			}
		}
...
<style scoped>
	...
	.zui-button__hover:active::after {
		content: '';
		position: absolute;
		width: 100%;
		height: 100%;
	...
	.zui-rightAngle::after {
		border-radius: 0 !important;
	}

	.zui-outline-rightAngle {
		border-radius: 0 !important;
	}

	.zui-outline-rightAngle::after {
		border-radius: 0 !important;
	}
