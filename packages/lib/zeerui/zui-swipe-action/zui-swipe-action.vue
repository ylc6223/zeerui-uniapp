<template>
	<view class="zui-swipeout-wrap" :style="{ backgroundColor: backgroundColor }">
		<view class="zui-swipeout-item" :class="[isShowBtn ? 'swipe-action-show' : '']"
			:style="{ transform: 'translate(' + position.pageX + 'px,0)' }">
			<view class="zui-swipeout-content" @touchstart="handlerTouchstart"
			...
			<view class="zui-swipeout-button-right-group" v-if="actions.length > 0" @touchend.stop="loop">
				<view class="zui-swipeout-button-right-item" v-for="(item, index) in actions" :key="index"
			...
			<view class="zui-swipeout-button-right-group" @touchend.stop="loop" @tap="handlerParentButton"
...
<style scoped>
	...
	.zui-swipeout-content {
		white-space: nowrap;
		overflow: hidden;
	}

	.zui-swipeout-button-right-group {
		position: absolute;
		right: -100%;
		top: 0;
		height: 100%;
	...
	.zui-swipeout-button-right-item {
		height: 100%;
		float: left;
		white-space: nowrap;
		box-sizing: border-box;
