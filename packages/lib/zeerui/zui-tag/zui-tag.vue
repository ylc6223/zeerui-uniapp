<template>
	<view class="zui-tag" :hover-class="hover ? 'zui-tag-opcity' : ''" :hover-stay-time="150"
		:class="[originLeft ? 'zui-origin-left' : '', originRight ? 'zui-origin-right' : '', getClassName(shape, plain), getTypeClass(type, plain)]"
		...
		@tap="handleClick">
		<slot></slot>
		<view class="zui-tag__border" :class="[getClassName(shape, plain)]" :style="{borderColor:getBgColor(type)}"
			v-if="plain"></view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			getTypeClass: function(type, plain) {
				return plain ? 'zui-' + type + '-outline' : 'zui-' + type;
			},
			getClassName: function(shape, plain) {
				//circle, square，circleLeft，circleRight
				var className = plain ? 'zui-tag-outline ' : '';
				if (shape != 'square') {
					if (shape == 'circle') {
						className = className + (plain ? 'zui-tag-outline-fillet' : 'zui-tag-fillet');
					} else if (shape == 'circleLeft') {
						className = className + 'zui-tag-fillet-left';
					} else if (shape == 'circleRight') {
						className = className + 'zui-tag-fillet-right';
					}
				}
				return className;
			}
...
<style scoped>
	...
	.zui-white.zui-tag-fillet {
		border-radius: 160rpx;
	}

	.zui-tag-outline-fillet {
		border-radius: 160rpx;
	}
